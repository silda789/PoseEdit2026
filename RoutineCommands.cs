...
namespace PoseEdit2026
{
    public static class RoutineCommands
    {
        ...
        //      выставляется в 50/3≈16.667 (было 25) - расчёт под требование "везде 2.5мм на
        //      бумаге", см. комментарий у константы newAttributeHeight ниже.
        ...
        public static void FixStandardTemplateLayersAndStyleCommand()
        {
            ...
            // ВАЖНО (обновлено 2026-08-22 - пользователь уточнил требование: ВЕЗДЕ в системе
            // высота текста должна давать РОВНО 2.5мм на бумаге, независимо от масштаба
            // листа). Высота A-F атрибутов внутри шаблона - НЕЗАВИСИМОЕ от масштаба чертежа
            // значение, отдельное от XScale/ObliquingAngle стиля. QuantityTableGenerator.cs
            // вставляет блок PZ_XX с BlockReference.ScaleFactors = (sut1_02/450, sat1_02/100),
            // где sat1_02 = olcek*1.50 - т.е. вертикальный масштаб блока = olcek*0.015. Текст
            // ОСНОВНОЙ таблицы теперь рисуется с высотой yzy2 = olcek*0.25 (см.
            // QuantityTableGenerator.cs, тот же коммит - пересчитано под 2.5мм на бумаге).
            // Чтобы после масштабирования блока итоговая высота атрибута совпала с высотой
            // текста таблицы ПРИ ЛЮБОМ масштабе (olcek сокращается):
            // templateHeight * 0.015 == 0.25 => templateHeight == 50/3 ≈ 16.667.
            const double newAttributeHeight = 50.0 / 3.0;
