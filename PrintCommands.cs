    ...
    public static class PrintCommands
    {
        ...
        public static void MagicPrintCommand()
        {
            ...
            try
            {
                ...
                // 2026-08-20).
                //
                // ВАЖНО (баг найден и исправлен 2026-08-20, реальный тест сразу после
                // добавления этого шага): db.SaveAs(dwgPath, ...) - НЕПРАВИЛЬНЫЙ способ
                // сохранить ИМЕННО ТЕКУЩИЙ ОТКРЫТЫЙ документ на его же файл - падает с
                // "Autodesk.AutoCAD.Runtime.Exception: eFilerError", потому что сам
                // AutoCAD уже держит файловый хэндл на этот файл, пока документ открыт
                // (низкоуровневый Database.SaveAs пытается открыть СВОЙ отдельный filer
                // поверх уже занятого файла - конфликт). Database.SaveAs годится, только
                // если сохраняешь в ДРУГОЙ файл/чужую Database, а не файл текущего
                // открытого документа. Правильный способ тихо сохранить именно активный
                // документ в его же файл - тот же путь, что и штатный Ctrl+S: команда
                // QSAVE, отправленная через SendStringToExecute - она работает через
                // внутренний механизм самого AutoCAD, который уже правильно владеет этим
                // файловым хэндлом, и не спрашивает имя/путь (это делает только SAVEAS
                // для НЕ сохранённого раньше чертежа, а QSAVE у уже именованного файла -
                // всегда без диалога). "_.QSAVE " - подчёркивание впереди задаёт
                // английское имя команды независимо от локализации интерфейса, точка
                // отключает возможную переопределённую пользователем команду с тем же
                // именем.
