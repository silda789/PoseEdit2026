...
namespace PoseEdit2026
{
    public static class PrintCommands
    {
        ...
        public static void MagicPrintCommand()
        {
            ...
            // Тихое сохранение чертежа ПОСЛЕ печати - подписка на EndPublish объявлена
            // здесь (не сразу внутри try), чтобы её можно было гарантированно отписать в
            // finally ниже даже если что-то упадёт по пути.
            //
            // ВАЖНО (два бага подряд на эту тему, оба 2026-08-20, по жалобе пользователя
            // "запрос на сохранение всё равно был" при закрытии AutoCAD после печати):
            // 1) db.SaveAs(dwgPath, ...) на ТЕКУЩЕМ ОТКРЫТОМ документе падает с
            //    "eFilerError" - AutoCAD сам держит файловый хэндл на этот файл, пока
            //    документ открыт, а низкоуровневый Database.SaveAs пытается открыть СВОЙ
            //    отдельный filer поверх уже занятого файла. Исправлено переходом на
            //    команду QSAVE (тот же путь, что и Ctrl+S - см. ниже).
            // 2) Даже через QSAVE диалог "Save Changes?" при закрытии AutoCAD ВСЁ РАВНО
            //    появлялся. Причина - вызывали SendStringToExecute("_.QSAVE") ДО
            //    PublishExecute: команда лишь встаёт в очередь ввода AutoCAD и реально
            //    выполняется позже, а Publish - процесс печати - способен сам
            //    дозаписывать что-то в чертёж (например, сведения о последней печати
            //    каждого Layout) уже ПОСЛЕ того, как мы поставили QSAVE в очередь, так что
            //    сохранение могло проскочить раньше, чем чертёж стал "по-настоящему"
            //    финально изменён. Исправлено: вместо того чтобы гадать момент времени,
            //    подписываемся на РЕАЛЬНОЕ событие "печать полностью завершена" -
            //    Autodesk.AutoCAD.Publishing.Publisher.EndPublish (найдено через
            //    рефлексию accoremgd.dll - официального события "публикация полностью
            //    закончена" в этой версии API нет в IntelliSense/документации под рукой,
            //    но оно есть в самой сборке) - и запускаем QSAVE уже ИЗ обработчика этого
            //    события, а не заранее вслепую.
            Autodesk.AutoCAD.Publishing.EndPublishEventHandler onEndPublish = null;
            ...
            try
            {
                ...
                // ВАЖНО (баг найден и исправлен 2026-08-20, реальный тест: LAYRENUM
                // переименовал листы, сразу следом запустили MAGICPRINT - Publish выдал
                // "ERROR: Layout not found" на ВСЕХ листах, 0 напечатано): DsdEntry ссылается
                // на лист по ИМЕНИ и по ПУТИ К ФАЙЛУ (DwgName = dwgPath), а Publish резолвит
                // это имя листа не через текущее состояние открытого документа в памяти, а
                // через то, что реально лежит НА ДИСКЕ - LAYRENUM переименовал листы только
                // в открытом документе, ничего не сохраняя, так что на диске всё ещё лежали
                // старые имена ("Layout1" и т.п.), и Publish просто не находил лист с новым
                // именем ("1", "2", ...). Раньше это не всплывало, потому что до сих пор
                // между любой правкой чертежа и запуском MAGICPRINT всегда происходило
                // сохранение (вручную или через QSAVE из прошлого запуска). Правильный
                // синхронный способ сохранить документ ИЗНУТРИ команды - не SendStringToExecute
                // (он лишь СТАВИТ команду в очередь, реально выполняется только после
                // возврата из текущей команды - см. историю бага про EndPublish ниже), а
                // Editor.Command(...) - блокирующий вызов, ждущий, пока команда реально
                // завершится, прежде чем вернуть управление сюда. Сохраняем ДО построения
                // DSD/вызова Publish, чтобы то, что видит Publish на диске, гарантированно
                // совпадало с тем, что мы только что прочитали в памяти (переименования
                // предыдущих команд вроде LAYRENUM тоже подхватятся этим сохранением).
                ed.Command("_.QSAVE");

                onEndPublish = (sender, e) =>
                {
                    // "_.QSAVE " - подчёркивание впереди задаёт английское имя команды
                    // независимо от локализации интерфейса, точка отключает возможную
                    // переопределённую пользователем команду с тем же именем. QSAVE у уже
                    // именованного файла всегда сохраняет без диалога (диалог "Сохранить
                    // как" показывает только SAVEAS/QSAVE НЕ сохранённого раньше чертежа).
                    doc.SendStringToExecute("_.QSAVE ", true, false, false);
                };
                Application.Publisher.EndPublish += onEndPublish;
            ...
            {
                if (onEndPublish != null)
                {
                    try { Application.Publisher.EndPublish -= onEndPublish; }
                    catch (System.Exception) { /* отписка не должна валить всю команду */ }
                }
