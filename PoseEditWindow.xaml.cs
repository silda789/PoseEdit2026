    ...
    public partial class PoseEditWindow : Window
    {
        ...
        private void InitLists()
        {
            try
            {
                ...
                // ====================================================================================
                // Заполняем список номерами форм от 01 до 93
                // "D2" означает формат с двумя цифрами (01, 02, ..., 93)
                // ВНИМАНИЕ (обновлено 2026-08-22): каталог пересчитан - "00" упразднён (01 теперь
                // единственный тип прямого стержня), "50" и "66" убраны как дубли (см. RebarRecognizer.
                // cs), остальные номера от 51/67 и выше сдвинуты - в папке Resources сейчас 93 картинки
                // (Shape_01.png .. Shape_93.png, без пропусков).
                ...
                var shapeNumbers = Enumerable.Range(1, 93) // Генерируем числа от 1 до 93 (93 элемента)
                    .Select(i => i.ToString("D2"))          // Преобразуем в строку с ведущим нулем (01, 02, ...)
        ...
        private void UpdateShapeImage()
        {
            ...
            // ====================================================================================
            // В папке Resources есть картинки от Shape_01.png до Shape_93.png (93 картинки,
            // обновлено 2026-08-22 - "00" упразднён, "50"/"66" убраны как дубли, см. RebarRecognizer.cs)
            // Если номер формы вне диапазона 1-93 или равен "99" (не распознано), картинки нет
            // Пытаемся преобразовать код в число для проверки
            if (int.TryParse(code, out int shapeNumber))
            {
                // Если номер вне 1-93 (включая устаревший "00"), картинки нет - очищаем изображение
                if (shapeNumber < 1 || shapeNumber > 93)
                {
