...
namespace PoseEdit2026
{
    ...
    public static class QuantityTableGenerator
    {
            ...
            List<RebarPositionInfo> toplamBilgi, double olcek, string lang)
        {
            ...
            // вызовов) должна давать РОВНО 3.0мм на бумаге при печати с учётом масштаба
            // листа, независимо от самого масштаба (уточнено пользователем 2026-08-22:
            // сначала просили 2.5, затем решили 3.0 - "давай высоту всех шрифтов не 2.5,
            // а 3.0 сделаем везде"). olcek = GetScale() * birim * 0.01 уже несёт в себе
            // масштаб листа, поэтому множитель 0.30 - единственное значение, которое даёт
            // ровно 3.0мм на бумаге при ЛЮБОМ масштабе (для 2.5мм было 0.25 - см.
            // PZSTDFIX/RoutineCommands.cs, там тоже пересчитано под 3.0).
