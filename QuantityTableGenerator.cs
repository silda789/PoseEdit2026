    ...
    public static class QuantityTableGenerator
    {
        ...
        private static void CreateMetrajLayers(Database db)
        {
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                ...
                // ВАЖНО (найдено пользователем 2026-08-22 - "толщина арматур опять Default, а
                // не 0.5мм"): толщина линии, зашитая PZSTDFIX'ом В ФАЙЛ ШАБЛОНА PZ_XX.dwg, НЕ
                // переносится сама по себе на уже существующий в ТЕКУЩЕМ чертеже слой с тем же
                // именем - слои общие для чертежа, а не "принадлежат" вставляемому блоку, и
                // блок при вставке не перезаписывает свойства чужого одноимённого слоя. Раньше
                // это выставлялось только для НОВОГО слоя (в цикле выше) - если слой уже
                // существовал (например, из более раннего запуска RQTN до этого фикса), его
                // LineWeight так и оставался тем, каким создался в первый раз. Поэтому здесь
                // принудительно выставляем 0.5мм на posedit.mtr.bar В ЛЮБОМ случае, а не только
                // при первом создании слоя.
                if (lt.Has("posedit.mtr.bar"))
                {
                    LayerTableRecord barLayer = (LayerTableRecord)tr.GetObject(lt["posedit.mtr.bar"], OpenMode.ForWrite);
                    barLayer.LineWeight = Autodesk.AutoCAD.DatabaseServices.LineWeight.LineWeight050;
                }
            ...
            List<RebarPositionInfo> toplamBilgi, double olcek, string lang)
        {
            ...
            // Высота ЛЮБОГО текста во всех трёх таблицах RQTN (yzy1/yzy2 - единственные
            // значения, которые передаются в Text_/T2 как "height" - см. полный список
            // вызовов) должна давать РОВНО 2.5мм на бумаге при печати с учётом масштаба
            // листа, независимо от самого масштаба (по прямому требованию пользователя
            // 2026-08-22, "везде в системе высота всех текстов должна быть равна 2.5").
            // olcek = GetScale() * birim * 0.01 уже несёт в себе масштаб листа, поэтому
            // множитель 0.25 - это единственное значение, которое даёт ровно 2.5мм на
            // бумаге при ЛЮБОМ масштабе (было 0.30, что при 1:50 в этой системе единиц
            // давало 150 модельных → 3мм на бумаге, а не 2.5 - именно расхождение, из-за
            // которого нашли и высоту атрибутов эскизов PZ_XX, см. PZSTDFIX/RoutineCommands.cs).
            double yzy1 = olcek * 0.25;
            double yzy2 = olcek * 0.25;
